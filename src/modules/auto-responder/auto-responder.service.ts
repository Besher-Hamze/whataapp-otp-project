...
export class AutoResponderService {
  ...
  try {
    ...
    this.logger.log(`Found matching rule with keywords "${matchingRule.keywords.join(', ')}" for message "${message}"`); // Updated to keywords array
    ...
    this.logger.log(`Auto-response sent to ${sender} using rule with keywords "${matchingRule.keywords.join(', ')}"`); // Updated to keywords array
    return true;
  } catch (error) {
